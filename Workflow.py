...
class Workflow:
    ...
    def generate(self, state):
        logger.info("---GENERATE TEST SCENARIOS AND TRACEABILITY MATRIX---")
        requirements = state["question"]
        documents = state["documents"]
        count = 0 if state["count"] is None else (state["count"] + 1)

        # Generate test scenarios using RAG chain
        scenarios = self.rag_chain.invoke({"context": documents, "question": requirements})
    ...
    def create_traceability_matrix(self, state):
        logger.info("---CREATE TRACEABILITY MATRIX---")
        requirements = state["question"]
        test_cases = state["generation"]

        # Simplified logic to map each test case to the corresponding requirement
        traceability_matrix = {}
        for req in requirements.splitlines():
            for test in test_cases.splitlines():
                if self.match_requirement_to_test_case(req, test):  # Replace with actual logic to match
                    traceability_matrix[req] = test

        return traceability_matrix

    def match_requirement_to_test_case(self, requirement, test_case):
        # Basic matching logic (you can improve this)
        return requirement.lower() in test_case.lower()
