...
def display_messages():
    ...
    for i, (msg, is_user) in enumerate(st.session_state["messages"]):
        if isinstance(msg, list):  # Check if the message is a list of multiple parts
            for part_idx, part in enumerate(msg):
                message(part, is_user=is_user, key=f"{i}-{part_idx}")
                time.sleep(1.5)  # Adding a delay between parts to simulate multiple messages
        else:
